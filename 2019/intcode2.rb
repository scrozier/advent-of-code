...
class Intcode2
  ...
  def initialize(program_text, network_address, input_function, output_function, halt_function)
  ...
  def execute_instruction_at(pos)
    ...
    when 3
      ...
      input = nil
      input = @input_function.call(@my_addr)
    ...
    when 4
      p1 = parameter_at(@pos + 1, p1mode)
      @output_function.call(@my_addr, p1)
      return @pos + 2
    when 5
      p1 = parameter_at(@pos + 1, p1mode)
      p2 = parameter_at(@pos + 2, p2mode)
